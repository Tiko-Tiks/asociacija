...
BEGIN
  ...
  IF NOT FOUND THEN
    IF NOT EXISTS (
      ...
      WHERE p.org_id = v_meeting.org_id
        AND p.user_id = v_user_id
        AND p.is_active = true
        AND p.title ILIKE '%BOARD%'
...
BEGIN
  ...
  -- Calculate hash (using md5 - PostgreSQL built-in, nereikalauja extension)
  -- NOTE: md5() naudojamas vietoj digest() dėl Supabase pgcrypto extension suderinamumo problemų
...
BEGIN
  ...
  IF v_protocol.status = 'DRAFT' THEN
    ...
    FROM public.memberships m
    INNER JOIN public.orgs o ON o.id = m.org_id
    WHERE m.org_id = v_protocol.org_id
      AND m.user_id = v_user_id
      AND m.member_status = 'ACTIVE'
      AND m.role = 'OWNER'
      AND o.status = 'ACTIVE'
