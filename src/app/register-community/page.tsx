...
export default function RegisterCommunityPage() {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    const applicationData = {
      communityName: getValue('community-name'),
      contactPerson: getValue('contact-person'),
      email: getValue('email').toLowerCase(),
      description: getValue('description'),
      registrationNumber: getValue('registration-number'),
      address: getValue('address'),
      usagePurpose: getValue('usage-purpose'),
      timestamp: new Date().toISOString(),
    }

    if (!applicationData.communityName || !applicationData.email) {
    ...
    try {
      ...
      // New flow: Email sent with onboarding link
